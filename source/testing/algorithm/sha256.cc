...
TEST(SHA256, DeathTest) {
	struct sha256 dummy_instance;
	uint8_t dummy_result[SHA256_DIGEST_SIZE];

	EXPECT_DEATH(sha256_begin(nullptr),  TESTING_DEATH_STRING);

	EXPECT_DEATH(sha256_update(nullptr, nullptr, (size_t) 1), TESTING_DEATH_STRING);
	EXPECT_DEATH(sha256_update(&dummy_instance, nullptr, (size_t) 1), TESTING_DEATH_STRING);
