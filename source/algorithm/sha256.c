 ...
 * Internal Functions
 */
static void _sha256_compress_chunk(struct sha256_inst *inst);
 ...
 * External Functions
 */
void sha256_begin(struct sha256_inst *inst)
{
	CONTEND(inst != NULL);
...
void sha256_update(struct sha256_inst *inst, const void *data, size_t len)
{
	CONTEND(inst != NULL);
...
void sha256_finish(struct sha256_inst *inst, void *digest)
{
	CONTEND(inst != NULL);
	CONTEND(digest != NULL);
