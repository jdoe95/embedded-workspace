 ...
 * R(X) as the CRC.
 *
 * There is no authoritarian CRC standard. Variations include the divisor G(X), shifting direction,
 * initial remainder value, whether the output is inverted etc. The header provides a common
...
/* prepares calculator */
void crc32_mf_begin(struct crc32_mf_inst *inst, const struct crc32_mf_info *info);
void crc16_mf_begin(struct crc16_mf_inst *inst, const struct crc16_mf_info *info);
void crc8_mf_begin(struct crc8_mf_inst *inst, const struct crc8_mf_info *info);
void crc32_lf_begin(struct crc32_lf_inst *inst, const struct crc32_lf_info *info);
void crc16_lf_begin(struct crc16_lf_inst *inst, const struct crc16_lf_info *info);
...
void crc32_mf_update(struct crc32_mf_inst *inst, const void *data, size_t len);
void crc16_mf_update(struct crc16_mf_inst *inst, const void *data, size_t len);
void crc8_mf_update(struct crc8_mf_inst *inst, const void *data, size_t len);
void crc32_lf_update(struct crc32_lf_inst *inst, const void *data, size_t len);
void crc16_lf_update(struct crc16_lf_inst *inst, const void *data, size_t len);
...
uint32_t crc32_mf_finish(struct crc32_mf_inst *inst);
uint16_t crc16_mf_finish(struct crc16_mf_inst *inst);
uint8_t crc8_mf_finish(struct crc8_mf_inst *inst);
uint32_t crc32_lf_finish(struct crc32_lf_inst *inst);
uint16_t crc16_lf_finish(struct crc16_lf_inst *inst);
